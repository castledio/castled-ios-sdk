...
class CastledInboxCell: UITableViewCell {
    ...
    private var actualConstraint : NSLayoutConstraint?
    
    override func awakeFromNib() {
        super.awakeFromNib()
        actualConstraint = constraintImageHeightRatio
        setupViews()
        // Initialization code
    }
    func setupViews(){
    ...
    internal func configureCellWith(_ inboxObj: CastledInboxItem){
        ...
        viewIsRead.isHidden = inboxObj.isRead
        let multiplier = inboxObj.aspectRatio

        let newConstraint = actualConstraint!.constraintWithMultiplier(multiplier)
...
//        viewContainer.layoutIfNeeded()
        ...
        if urlImageString.count > 0,let url = URL(string: urlImageString) {
         imgCover.sd_setImage(with: url, placeholderImage: placeholderImage)
        }
        else
        {
