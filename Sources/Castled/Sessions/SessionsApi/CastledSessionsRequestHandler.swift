//
//  CastledSessionsRequestHandler.swift
//  Castled
//
...
class CastledSessionsRequestHandler: CastledNetworkRequestHandler {
    static func handleRequest(requests: [CastledNetworkRequest], onSuccess: @escaping ([CastledNetworkRequest]) -> Void, onError: @escaping ([CastledNetworkRequest]) -> Void) {
        ...
        let newRequest = CastledSessionRepository.getEventsRequest(params: batchedEvents)
        CastledNetworkLayer.shared.makeApiCall(request: newRequest, path: CastledSessionRepository.eventsPath) { response in
            if response.success {
                onSuccess(requests)
            } else {
                onError(requests)
