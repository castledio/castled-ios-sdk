//
//  CastledDeviceInfoRequestHandler.swift
//  Castled
//
...
class CastledDeviceInfoRequestHandler: CastledNetworkRequestHandler {
    static func handleRequest(requests: [CastledNetworkRequest], onSuccess: @escaping ([CastledNetworkRequest]) -> Void, onError: @escaping ([CastledNetworkRequest]) -> Void) {
        ...
        requests.forEach { request in
            ...
            castledGroup.enter()
            CastledNetworkLayer.shared.makeApiCall(request: newRequest, path: CastledDeviceInfoRepository.eventsPath) { response in
                if response.success {
