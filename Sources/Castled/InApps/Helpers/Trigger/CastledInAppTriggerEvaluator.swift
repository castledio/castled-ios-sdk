        ...
        for filter in propertyFilters {
            if let evaluator = getParamEvaluator(triggerType: filter.operation.propertyType.rawValue) {
                if evaluator.evaluateCondition(value: params?[filter.name], propertyOperation: filter.operation) == false {
                    if showLog == true {
                        CastledLog.castledLog("Unable to satisfy the trigger condition: \(filter.name) for \(String(describing: params?[filter.name]))", logLevel: CastledLogLevel.error)
                    }
                    return false
                }
            } else {
                if showLog == true {
                    CastledLog.castledLog("No evaluator defined for property type: \(filter.operation.type)", logLevel: CastledLogLevel.error)
                }
                return false
            }
        }
    ...
    private func evaluateOr(propertyFilters: [CIEventFilters], params: [String: Any]?, showLog: Bool? = true) -> Bool {
        for filter in propertyFilters {
            if let evaluator = getParamEvaluator(triggerType: filter.operation.propertyType.rawValue) {
                ...
                if showLog == true {
                    CastledLog.castledLog("No evaluator defined for property type: \(filter.name)", logLevel: CastledLogLevel.error)
                }
            }
        }
        return false
